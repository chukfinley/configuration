...
#hopt -s cdspell # autocorrects cd misspellings
...
#  if [ -n "$1" ]; then
#    if [ -d "$1" ]; then
#      find "$1" -type f -exec shred -n 4 -u {} \; > /dev/null 2>&1
      
#      if [ -z "$(find "$1" -type f -mindepth 1)" ] && [ -z "$(find "$1" -mindepth 1 -type d -not -empty)" ]; then
#        rm -rf "$1" > /dev/null 2>&1
#      fi
#    elif [ -f "$1" ]; then
#      shred -n 4 -u "$1" > /dev/null 2>&1
#    else
#      :
#    fi
#  fi
